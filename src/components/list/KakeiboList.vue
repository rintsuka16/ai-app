...
<script>
  export default {
    computed: {
      ...
      Items() {
        return this.$store.state.items.Items; // 名前空間の変更
      },
      Total() {
        return this.$store.getters['items/Total']; // 名前空間の変更
      },
      // getterから合計金額を取得
      SumOut() {
        return this.$store.getters['expenses/SumOut']; // 名前空間の変更
      },
      SumIn() {
